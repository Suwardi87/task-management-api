    ...
    <AppLayout title="Task List">
        ...
        <div class="space-y-6 m-20">
            ...
            <div class="flex justify-between mb-4">
                <Link :href="route('tasks.export.csv')"
                    ...
                    <tbody>
                        <tr v-for="task in filteredTasks" :key="task.id" class="border-t hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 space-x-2">
                                <Link :href="route('tasks.show', task.id)" class="text-green-600 hover:underline">Detail
                                </Link>
                                <Link :href="route('tasks.edit', task.id)" class="text-blue-600 hover:underline">Edit
                                </Link>
                            </td>
                        </tr>
                        <tr v-if="filteredTasks.length === 0">
                            <td colspan="5" class="text-center py-6 text-gray-400 italic">Tidak ada tugas ditemukan.
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="flex justify-center gap-2">
                <Link v-for="(link, i) in paginationLinks" :key="i" v-if="link" :href="link.url || '#'"
                    v-html="link.label" @click.prevent="link.url ? Inertia.visit(link.url) : null"
...
function exportCsv() {
  window.open(route('tasks.export.csv'), '_blank');
}
